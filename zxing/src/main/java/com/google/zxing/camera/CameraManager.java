...
//    private static final int MAX_FRAME_HEIGHT = 675; // = 5/8 * 1080
     ...
     * 扫码框的位置
     */
    public Rect getShowFrameRect() {
        Point screenResolution = configManager.getScreenResolution();
        if(screenResolution == null) {
            return null;
        }
        if (showFrameRect == null) {
            if (camera == null) {
                return null;
            }
            ...
            if(height >= width) { //竖屏
                ...
                topOffset = (screenResolution.y - width) / 2;
                showFrameRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + width);
            } else { //
                leftOffset = (screenResolution.x - height) / 2;
                topOffset = (screenResolution.y - height) / 2;
                showFrameRect = new Rect(leftOffset, topOffset, leftOffset + height, topOffset + height);
            }
        }
        return showFrameRect;
    }
